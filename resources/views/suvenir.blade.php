        ...
        <div
            ...
            <div class="flex justify-between">
                ...
                <div class="ml-auto">
                    ...
                    <button
                        class="border border-slate-800 py-2 px-3 rounded-full text-sm font-bold active:bg-slate-800 hover:bg-slate-500 hover:text-white"><a
                            href="{{ route('ingresarSuvenir') }}">Nuevo</a></button>
                </div>
            </div>
            @if (session('success'))
            ...
            @endif
            <h2 class="text-xl lg:text-2xl font-semibold text-center">Suvenires</h2>
                    ...
                    <tbody>
                        @foreach ($suvenir as $suvenir)
                            <tr class="border border-slate-400 hover:bg-slate-300">
                                <td class="p-2 min-w-44">{{ $suvenir->codigo }}</td>
                                <td class="p-2 min-w-44">{{ $suvenir->nombre }}</td>
                                <td class="p-2 min-w-28">Q.{{ $suvenir->precio }}</td>
                                <td>
                                    <button
                                        ...
                                        <a href="{{ route('editarSuvenir', $suvenir->codigo) }}"><img
                                                src="{{ asset('/img/editar.png') }}" class="w-5" alt=""></a>
                                    </button>
                                </td>
                                <td>
                                    <form action="{{ route('eliminarSuvenir', $suvenir->codigo) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            onclick="return confirm('¿Estás seguro de que deseas eliminar ( {{ $suvenir->nombre }} )?')"
            ...
            <div class="flex justify-center gap-1 pt-5 flex-col">
                ...
                <div class="flex items-center gap-2">
                    <form method="GET" action="{{ route('suvenires', $suvenir) }}">
